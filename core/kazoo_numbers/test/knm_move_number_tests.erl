...
move_to_child() ->
    {'ok', PN} = knm_numbers:move(?TEST_AVAILABLE_NUM, ?CHILD_ACCOUNT_ID),
...
acquire_parent_reserved() ->
    ...
    {'ok', PN1} = knm_numbers:move(Num, To, [{auth_by, To}]),
    {'ok', PN2} = knm_numbers:move(Num, To),
...
acquire_parent_reserved_failure() ->
    ...
    {'ok', PN0} = knm_numbers:get(Num),
    {'error', Error1} = knm_numbers:move(Num, To, [{auth_by, To}]),
    {'ok', PN1} = knm_numbers:move(Num, To),
    [?_assert(not knm_phone_number:is_dirty(PN0))
    ,{"verify test number is assigned to reseller"
     ,?_assertEqual(?RESELLER_ACCOUNT_ID, knm_phone_number:assigned_to(PN0))
     }
...
move_changing_public_fields() ->
    ...
    {ok, PN0} = knm_numbers:get(?TEST_AVAILABLE_NUM),
    {ok, PN} = knm_numbers:move(?TEST_AVAILABLE_NUM, ?RESELLER_ACCOUNT_ID, Options),
    [?_assert(knm_phone_number:is_dirty(PN))
...
move_available_non_local() ->
    {ok, PN0} = knm_numbers:get(?TEST_AVAILABLE_NON_LOCAL_NUM),
    [?_assert(not knm_phone_number:is_dirty(PN0))
    ,{"Verify number is available"
     ,?_assertEqual(?NUMBER_STATE_AVAILABLE, knm_phone_number:state(PN0))
     }
...
everyone_is_allowed_to_buy_available(Num, AuthBy, AssignTo, Carrier, Features) ->
    {ok, PN} = knm_numbers:move(Num, AssignTo, [{auth_by,AuthBy}]),
    [?_assert(knm_phone_number:is_dirty(PN))
