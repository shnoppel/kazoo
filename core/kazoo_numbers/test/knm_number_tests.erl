...
available_as(AuthAccountId) ->
    case knm_numbers:get(?TEST_AVAILABLE_NUM, [{'auth_by', AuthAccountId}]) of
...
get_unreconcilable_number() ->
    [{"Verify non-reconcilable numbers result in errors"
     ,?_assertMatch({'error', 'not_reconcilable'}, knm_numbers:get(<<"1000">>))
     }
    ].

get_not_found() ->
    [?_assertEqual({error, not_found}, knm_numbers:get(<<"4156301234">>))
    ].

mdn_transitions() ->
    Num = ?TEST_IN_SERVICE_MDN,
    DefaultOptions = [{assign_to, ?MASTER_ACCOUNT_ID} | knm_options:mdn_options()],
    {ok, PN1} = knm_numbers:move(Num, ?MASTER_ACCOUNT_ID, DefaultOptions),
    {ok, PN2} = knm_numbers:release(Num, DefaultOptions),
    {ok, PN4} = knm_numbers:create(?TEST_CREATE_NUM, [{module_name,?CARRIER_MDN}|DefaultOptions]),
    ...
    ,{"Verify MDN cannot be reserved"
     ,?_assertMatch({error,_}, knm_numbers:reserve(Num, knm_options:default()))
     }
...
is_mdn_for_mdn_run() ->
    ...
    [{"Verify an mdn_run && knm_mdn number can be updated"
     ,?_assertMatch({ok,_}, knm_numbers:update(?TEST_IN_SERVICE_MDN, Fs, [Run|Base]))
     }
    ,{"Verify an mdn_run && !knm_mdn number cannot be updated"
     ,?_assertMatch({error,_}, knm_numbers:update(?TEST_IN_SERVICE_NUM, Fs, [Run|Base]))
     }
    ,{"Verify a !mdn_run && knm_mdn number cannot be updated"
     ,?_assertMatch({error,_}, knm_numbers:update(?TEST_IN_SERVICE_MDN, Fs, Base))
     }
...
assign_to_app() ->
    Num = ?TEST_IN_SERVICE_NUM,
    MyApp = <<"my_app">>,
    {ok, PN0} = knm_numbers:get(Num),
    {ok, PN1} = knm_numbers:assign_to_app(Num, MyApp),
...
fix_number_wrong_used_by_and_dangling_pvt_features() ->
    {ok, PN1} = knm_numbers:get(?TEST_OLD7_NUM),
    [PN2] = knm_pipe:succeeded(fix_number(PN1)),
    [?_assert(not knm_phone_number:is_dirty(PN1))
