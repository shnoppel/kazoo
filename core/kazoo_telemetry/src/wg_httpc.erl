...
%% @end
%%------------------------------------------------------------------------------
-spec post(kz_json:object()) -> kz_http:httpc_ret().
post(PingObj) ->
    Type = kz_json:get_value(<<"type">>, PingObj),
    Response = kz_http:post(endpoint_url(Type), default_headers(), PingObj),
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_response(kz_http:httpc_ret()) -> {'ok', kz_json:object()}
...
%% @end
%%------------------------------------------------------------------------------
-spec user_agent() -> kz_term:ne_binary().
...
%% @end
%%------------------------------------------------------------------------------
-spec version() -> kz_term:ne_binary().
version() -> ?WG_VERSION.
